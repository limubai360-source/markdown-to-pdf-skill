...
- 📑 **目录生成** - 支持 TOC 扩展
- 🔄 **批量转换** - 支持目录批量处理
- 🎨 **自定义样式** - 支持自定义 CSS 作为默认样式后的覆盖层
- 🧾 **正式报告排版** - 正文首行缩进 2 字符、清晰标题层级、A4 页眉页码
...
# 指定页眉标题
python md2pdf.py report.md --title "正式汇报材料"
```
...
| `--css` | 自定义 CSS 文件；会追加在默认正式报告样式之后，用作覆盖层 |
| `--title` | 文档标题；默认用于 PDF 元信息和页眉左侧标题 |
...
默认样式面向中文正式报告：A4 页面，页眉左侧显示文档标题，页脚居中显示 `当前页 / 总页数`；正文采用首行缩进 2 字符、约 11.5pt 字号和 1.7 行高；标题、列表、引用、表格、代码块有独立层级样式。

有序列表会自动处理“编号标题 + 空行 + 正文”的松散写法，避免 PDF 中多个编号项都显示为 `1.`。
...
| 有序列表全部显示为 1 | 使用新版脚本重新转换；默认会归一化松散有序列表 |
| 页眉标题不符合预期 | 使用 `--title` 指定文档标题 |
| 内存不足 | 尝试分批转换大文件 |
