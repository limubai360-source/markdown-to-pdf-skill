...
| **目录生成** | 支持 TOC 扩展生成目录 |
| **批量转换** | 支持目录批量处理 |
| **自定义样式** | 支持自定义 CSS 文件作为默认样式后的覆盖层 |
| **正式报告样式** | 默认采用中文报告排版：正文首行缩进 2 字符、清晰标题层级、A4 页眉页码 |
| **页眉页码** | 默认页眉左侧显示文档标题，页脚居中显示 `当前页 / 总页数` |
| **编号修正** | 默认有序列表连续编号，并自动处理编号标题后跟空行和正文的松散列表，避免 PDF 中全部显示为 1 |
...
| 有序列表全部显示为 1 | 使用新版脚本重新转换；默认已移除会保留起始编号的 `sane_lists` 扩展，并会自动归一化松散有序列表 |
